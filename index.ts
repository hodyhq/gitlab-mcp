...
async function createOrUpdateFile(
  ...
  const body: Record<string, any> = {
    branch,
    // hodyhq fork: base64 content is passed through untouched (binary-safe);
    // otherwise fall back to the global text/base64 convenience toggle.
  ...
  const response = await fetch(url.toString(), {
    ...
    body: JSON.stringify({
      ...
      actions: actions.map(action => {
        // hodyhq fork: honour per-file action (create/update/delete/move) instead
        // of hardcoding "create", and pass base64 content through untouched so
